...
export default function EsfImportPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      const res = await fetch("/api/esf/import", {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        throw new Error(body?.error ?? "Не удалось импортировать ЭСФ");
      }

      const data = await res.json();
      const id = data.id ?? data.esfId;
    ...
    <AppShell activePage="ЭСФ">
      <div style={{ padding: "2rem", maxWidth: 720, margin: "0 auto" }}>
        <div style={{ marginBottom: "1.5rem" }}>
          ...
          <h1 style={{ margin: "0.25rem 0 0" }}>Импорт ЭСФ</h1>
        </div>
